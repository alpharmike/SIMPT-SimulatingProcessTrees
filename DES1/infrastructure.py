...
import time
from pm4py.algo.simulation.playout.process_tree.variants import extensive
import eventlet

# log = pm4py.read_xes("BPI_Challenge_2012_APP.xes")
from pm4py.algo.simulation.playout import process_tree as tree_playout
...
from pm4py.objects.process_tree import obj as ptree
...
class recieve_and_convert_log:
    ...
    def extract_timewindow(self, start, end):
        ...
        try:
            filtered_log = timestamp_filter.filter_traces_contained(self.event_log, start, end,
                                                                    parameters={
                                                                        timestamp_filter.Parameters.TIMESTAMP_KEY: "time:timestamp_start"})
            filtered_log = timestamp_filter.filter_traces_contained(filtered_log, start, end,
                                                                    parameters={
                                                                        timestamp_filter.Parameters.TIMESTAMP_KEY: "time:timestamp_complete"})
        except:
            filtered_log = timestamp_filter.filter_traces_contained(self.event_log, start, end)
    ...
    @classmethod
    def get_simulatorinformation(self, log, capacity, activitiescapacity, businesshour, businessday, stop,
                                 activitylimit, tracelimit, miss, limittime):
        activitiesList = []
    ...
    @classmethod
    def simulate_log(self, csv_writer, tree, duration, startID, starttime, waitingtime, arrivallist, deviation, info,
                     numtrace, start, arradeviainday, converttree):
        global FLAG
        ...
        env.process(process.setup(env, csv_writer, startID, starttime, tracesList, duration, waitingtime, arrivallist,
                                  deviation, info, arradeviainday, simres, self.actwaitdict))
    ...
    def get_endtime(self, log):
        ...
        if len(timelist) == 0:
            ...
            starttime = str(firstelement)[0:4] + '-' + str(firstelement)[4:6] + '-' + str(firstelement)[
                                                                                      6:8] + ' ' + str(firstelement)[
                                                                                                   8:10] + ':' + str(
                firstelement)[10:12] + ':' + str(firstelement)[12:14]
            endtime = str(lastelement)[0:4] + '-' + str(lastelement)[4:6] + '-' + str(lastelement)[6:8] + ' ' + str(
                lastelement)[8:10] + ':' + str(lastelement)[10:12] + ':' + str(lastelement)[12:14]
    ...
    def evaluation(self, activity_duration):
        ...
        evaluationlist.append(
            'There are %s cases happened, waited %s times, got interrupted %s times and missed %s times' % (
            evaluation[3], evaluation[4], evaluation[5], evaluation[7]))
    ...
    def statics(self, log):
        ...
        return (
        numtrace, numactivity, activitylist, timeinterval, (meantraceduration, tracedeviation), case_arrival_ratio,
    ...
    def initiallimit(self, log):
        ...
        cycletime = int(
            (dt.datetime.strptime(interval[1], fmt) - dt.datetime.strptime(interval[0], fmt)).total_seconds())
    ...
    def simulateresource(self, originlog, log, logname, logres):
        ...
        actcluster = roles_discovery.apply(originlog, variant=None,
                                           parameters={rpd.Parameters.RESOURCE_KEY: self.logreso})
        simreslog = []
        for trace in log:
            simrestrace = []
            for event in trace:
                if event == trace[0]:
                    for ele0 in actcluster:
                        if event[logname] in ele0.activities:
                            sum = 0
                            choosedict = [(0, '')]
                            for key, value in ele0.originator_importance.items():
                                sum += value
                    ...
                    for ele0 in actcluster:
                        if event[logname] in ele0.activities:
                            executor = ele0.originator_importance.keys()
                    dominator = 0
                    sample = []
                    ispre = False
                    i134 = -1
    ...
    def convertptree(self, intree, parenttree, root):
        ...
        if ptsp[0] == '->(':
            retree._operator = Operator.SEQUENCE
            retree._label = None
            next = ptsp[1:-1]
            # retree._children = []
            # print(next,'line 878')
            ...
            retree._operator = Operator.PARALLEL
            retree._label = None
            next = ptsp[1:-1]
            # retree._children = []
            # print(next,'line 884')
            ...
            retree._operator = Operator.OR
            retree._label = None
            next = ptsp[1:-1]
        elif ptsp[0] == '*(':
            retree._operator = Operator.LOOP
            retree._label = None
            next = ptsp[1:-1]
            # retree._children = []
            # retree._children = childlist.append(self.convertptree(ptsp[1:-1],retree,[]))
        elif ptsp[0] == 'X(':
            retree._operator = Operator.XOR
            retree._label = None
            next = ptsp[1:-1]
            # retree._children = []
            # print(next,'line 898')
    ...
    def countrepeat(self, tree, countloop, actnum):
        ...
        while not tree in countloop.keys():
            if tree.operator == Operator.PARALLEL or tree.operator == Operator.SEQUENCE:
                countlist = []
                for child in tree.children:
                ...
                for trace in log:
                    for i in range(len(trace) - 1):
                        if trace[i][self.logname] not in timedict.keys():
                            timedict[trace[i][self.logname]] = [
                                (str(trace[i][self.logtime])[0:19], str(trace[i + 1][self.logtime])[0:19])]
                            # print("hi",trace[i][self.logname],timedict[trace[i][self.logname]])
                        else:

                            timedict[trace[i][self.logname]].append(
                                (str(trace[i][self.logtime])[0:19], str(trace[i + 1][self.logtime])[0:19]))
    ...
    def evaluatetree(self, tree, maxlooplist, actdict, initialvalue, evaluatetreelist):
        ...
        if tree.operator is Operator.LOOP:
            ...
            for child in tree.children:
                self.evaluatetree(child, maxlooplist, actdict, evaluatetreelist[child], evaluatetreelist)
        elif tree.operator is Operator.OR:
            print("line 213")
            for child in tree.children:
                childlist = []
                self.findlabel(child, childlist)
            ...
            for child in tree.children:
                self.evaluatetree(child, maxlooplist, actdict, evaluatetreelist[child], evaluatetreelist)
        elif tree.operator is Operator.PARALLEL or tree.operator is Operator.SEQUENCE:
            # print("line 224")
            for child in tree.children:
    ...
    def evaluatenode(self, node, actdict):
        if node.operator is Operator.PARALLEL or node.operator is Operator.SEQUENCE:
            value = 0
            childlist = []
        ...
        elif node.operator is Operator.XOR or node.operator is Operator.OR:
            value = 0
            childlist = []
            self.findlabel(node, childlist)
            for child in childlist:
    ...
    def gettreeinfo(self):
        ...
        for key in evaluatetreelist.keys():
            ...
            if not key.parent is None:
                if key.parent.operator == Operator.XOR and (not key.children == [] or not key.label is None):
